...
object Server {
  ...
  val reframedFlow: Flow[ByteString, String, NotUsed] =
    Framing.delimiter(ByteString.apply("\n"), 100).via(byteStringToStringFlow)

  private def byteStringToStringFlow:Flow[ByteString, String, NotUsed] = Flow.fromFunction(_.utf8String)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] = {
    Flow[Event].statefulMapConcat { () =>
      ...
      event => if (event.sequenceNr == nextSeq) {
        buffer += event.sequenceNr -> event

        var followingSeqs = Seq.empty[Int]

        while (buffer.contains(nextSeq)) {
          followingSeqs = followingSeqs :+ nextSeq
          nextSeq += 1
        }

        val events = followingSeqs.map(i => buffer(i))
        buffer = buffer -- followingSeqs
        events.toList
      } else {
        buffer += event.sequenceNr -> event
        List()
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var followers: Followers = Map().withDefaultValue(Set())
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = {
    ...
    event match {
      case Follow(sequenceNr, fromUserId, toUserId) =>
        userId == toUserId
      case Unfollow(sequenceNr, fromUserId, toUserId) =>
        false
      case Broadcast(_) =>
        true
      ...
      case StatusUpdate(_, fromUserId) =>
        val followings = followers(userId)
        followings.contains(fromUserId)
    }
  }
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue { f =>
        f.foreach(clientIdPromise.success)
        NotUsed
      }
