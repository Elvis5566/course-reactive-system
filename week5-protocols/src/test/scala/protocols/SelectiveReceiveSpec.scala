...
class SelectiveReceiveSpec extends FunSuite with PropertyChecks with MustMatchers {
    ...
    def expectOne[T](inbox: TestInbox[T], seq: List[T]): Behavior[T] = {
      println("expectOne seq: " + seq)
      seq match {
        case x :: xs =>
          receiveMessagePartial {
            case `x` =>
              println("expectOne x: " + x)
              inbox.ref ! x
              expectOne(inbox, xs)
          }
...
//              println("start: " + x)
  ...
  test("A SelectiveReceive Decorator must pass") {
    val values = List("A", "B", "C")
    val i = TestInbox[String]()
    val b = behavior(i, 3, values)
    val testkit = BehaviorTestKit(b, "simple")
  ...
  test("A SelectiveReceive Decorator must pass2") {
    val values = List("A", "B", "C")
    val i = TestInbox[String]()
    val b = behavior(i, 3, values)
    val testkit = BehaviorTestKit(b, "simple")
