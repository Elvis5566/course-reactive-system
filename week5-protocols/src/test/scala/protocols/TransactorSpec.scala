...
class TransactorSpec extends FunSuite with MustMatchers with PropertyChecks {
  ...
  test("A Transactor must timeout") {
    ...
    testkit.runOne()
    val ref :: Nil = sessionInbox.receiveAll()
    val Effect.SpawnedAnonymous(_, _) :: Effect.Watched(`ref`) :: g :: Nil = testkit.retrieveAllEffects()
    info("ggggg " + g)
    ref must be(testkit.childInbox(ref.path.name).ref)
    val session = testkit.childTestKit(ref)
  ...
  // It looks like if we wanna test crash, we have to use async test kit
//  test("watch trigger") {
//    val done = TestInbox[String]()
//
//    val start = 1
//    val testkit = BehaviorTestKit(Transactor(start, 1.seconds).asInstanceOf[Behavior[PrivateCommand[Int]]])
//
//    val sessionInbox = TestInbox[ActorRef[Session[Int]]]()
//    testkit.ref ! Begin(sessionInbox.ref)
//    testkit.runOne()
...
//    done.receiveAll() must be(Seq("done"))
//
//    session.ref ! Modify((_: Int) / 0, 1, "done", done.ref)
//    session.runOne()
//
//    info("gooood" + testkit.selfInbox.receiveAll())
//
//  }
