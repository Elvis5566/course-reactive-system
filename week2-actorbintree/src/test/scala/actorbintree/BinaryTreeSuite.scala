...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  def receiveN(requester: TestProbe, ops: Seq[Any], expectedReplies: Seq[OperationReply]): Unit =
    requester.within(5.seconds) {
      val repliesUnsorted = for (i <- 1 to ops.size) yield try {
        val reply = requester.expectMsgType[OperationReply]
        info("reply " + reply)
        reply
      } catch {
  ...
  def verify(probe: TestProbe, ops: Seq[Any], expected: Seq[OperationReply]): Unit = {
    ...
    receiveN(probe, ops.filter(_.isInstanceOf[GeneralOperation]), expected)
  ...
  test("preorder example") {
    ...
    val ops = List(
      ...
      Insert(requesterRef, id=20, 2),
      Insert(requesterRef, id=30, 5),
      Insert(requesterRef, id=40, 4),
      InOrder(requesterRef, id=1000),
      Remove(requesterRef, id=1100, 4),
      InOrder(requesterRef, id=1200)
    )
    ...
    val expectedReplies = List(
      ...
      OperationFinished(id=100),
      InOrdered(id=1000, List(1,2,3,4,5)),
      OperationFinished(id=1100),
      InOrdered(id=1200, List(1,2,3,5))
    )
    verify(requester, ops, expectedReplies)
  }
  ...
  test("get height") {
    ...
    val ops = List(
      ...
      Insert(requesterRef, id=30, 5),
      Insert(requesterRef, id=40, 4),
      GetHeight(requesterRef, id = 1000)
    )
...
//      GetHeight(requesterRef, id = 1000),
//      GC,
//      GetHeight(requesterRef, id = 2000)
//    )
//
//    val expectedReplies = List(
//      OperationFinished(id=10),
